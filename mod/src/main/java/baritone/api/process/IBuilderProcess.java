...
public interface IBuilderProcess extends IBaritoneProcess {
    ...
    default boolean build(String schematicFile, BlockPos origin) {
        File file = new File(new File(net.minecraftforge.fml.loading.FMLPaths.GAMEDIR.get().toFile(), "schematics"), schematicFile);
        return build(schematicFile, file, origin);
    }

    // buildOpenSchematic removed (client-only)

    // buildOpenLitematic removed (client-only)

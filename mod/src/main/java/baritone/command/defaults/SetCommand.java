...
// Minecraft client import removed (server-side)
        ...
        if (args.hasAny()) {
            ...
            if (args.hasExactlyOne() && !Arrays.asList("s", "save").contains(args.peekString().toLowerCase(Locale.US))) {
                if (arg.equalsIgnoreCase("reset")) {
                    ...
                    return RelativeFile.tabComplete(args, net.minecraftforge.fml.loading.FMLPaths.GAMEDIR.get().resolve("baritone").toFile());
                }

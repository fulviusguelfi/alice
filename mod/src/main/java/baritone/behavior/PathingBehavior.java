...
// ElytraProcess removed (client-only)
// PathRenderer removed (client-only rendering)
...
public final class PathingBehavior extends Behavior implements IPathingBehavior, Helper {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        if (current != null) {
            switch (event.getState()) {
                case PRE:
                    // Server-side: no auto-jump option to manage
                    break;
                case POST:
                    break;
                default:
    ...
    public boolean isSafeToCancel() {
        if (current == null) {
            return true; // ElytraProcess removed (server-side)
        }
        return safeToCancel;
    }
    ...
    public void onRenderPass(RenderEvent event) {
        // PathRenderer.render removed (client-only rendering)
    }
}

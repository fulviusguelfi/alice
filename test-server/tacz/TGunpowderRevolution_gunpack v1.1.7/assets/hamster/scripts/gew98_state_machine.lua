...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_BAYONET_MUZZLE) then
        ...
        context:runAnimation(animationName, context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.4)
        return main_track_states.idle
    end
    if (input == INPUT_BAYONET_STOCK) then
        local counter = main_track_states.bayonet_counter
        local animationName = "melee_stock_" .. tostring(counter + 1)
        main_track_states.bayonet_counter = (counter + 1) % 3
        context:runAnimation(animationName, context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.4)
        return main_track_states.idle
    end
...
function main_track_states.reload.entry(this, context)
    if (context:getMagExtentLevel() == 2) then
        if (isNoAmmo(context)) then
            main_track_states.reload.emptyload = 1
            context:runAnimation("reload_start_clip_empty", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.1)
        elseif(context:getAmmoCount() == 0) then
            context:runAnimation("reload_start_clip_1round", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.1)
        else
            context:runAnimation("reload_start_clip", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.1)
        end
    else
        if (isNoAmmo(context)) then
            main_track_states.reload.emptyload = 1
        ...
        if(context:isHolding(track)) then
            if (context:getMagExtentLevel() == 2) then
                if(main_track_states.reload.emptyload == 1) then
                    if (main_track_states.reload.need_ammo - main_track_states.reload.loaded_ammo >= 5 and context:getAttachment("SCOPE") == "tacz:empty") then
                        context:runAnimation("xmag_reload_loop_clip_5round", track, false, PLAY_ONCE_HOLD, 0)
                        main_track_states.reload.loaded_ammo = main_track_states.reload.loaded_ammo + 5
                    end
                    main_track_states.reload.emptyload = 0
                else
                    if (main_track_states.reload.need_ammo - main_track_states.reload.loaded_ammo >= 5 and context:getAttachment("SCOPE") == "tacz:empty") then
                        context:runAnimation("xmag_reload_loop_clip", track, false, PLAY_ONCE_HOLD, 0)
                        main_track_states.reload.loaded_ammo = main_track_states.reload.loaded_ammo + 5
                    else
                        context:runAnimation("xmag_reload_loop_clip_single", track, false, PLAY_ONCE_HOLD, 0)
                        main_track_states.reload.loaded_ammo = main_track_states.reload.loaded_ammo + 1
                    end
                end
            else
                if(main_track_states.reload.emptyload == 1) then
                    if (main_track_states.reload.need_ammo == 5 and context:getAttachment("SCOPE") == "tacz:empty") then
                        if (main_track_states.reload.loaded_ammo ~= main_track_states.reload.need_ammo) then
                            context:runAnimation("reload_load_clip_5round", track, false, PLAY_ONCE_HOLD, 0)
                        end
                        main_track_states.reload.loaded_ammo = main_track_states.reload.loaded_ammo + 5
                    else
...
function main_track_states.reload.transition(this, context, input)
    if (input == main_track_states.reload.retreat or input == INPUT_CANCEL_RELOAD) then
        if (context:getMagExtentLevel() == 2) then
            context:runAnimation("xmag_reload_load_clip_loop_end", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.1)
        else
            if (main_track_states.reload.emptyload == 1 and context:getAttachment("SCOPE") == "tacz:empty") then
                context:runAnimation("reload_end_clip", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.1)
            else

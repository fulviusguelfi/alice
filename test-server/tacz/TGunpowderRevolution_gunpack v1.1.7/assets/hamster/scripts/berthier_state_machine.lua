...
local function getBulletCount(context)
    return (context:hasBulletInBarrel() and 1 or 0) + context:getAmmoCount()
end
...
function bolt_caught_states.bolt_caught.entry(this, context)
    context:runAnimation("static_bolt_caught", context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), false, LOOP, 0.4)
end
...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_BOLT) then
        if(context:getAmmoCount() == 1) then
            context:runAnimation("bolt_last", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        else
...
function main_track_states.reload.entry(this, context)
    ...
    elseif(getBulletCount(context) == 1) then
        context:runAnimation("reload_start_1round", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
    elseif(getBulletCount(context) == 2) then
        context:runAnimation("reload_start_2round", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
    else
        context:runAnimation("reload_start_3round", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
    end
    main_track_states.reload.need_ammo = 0
end

function main_track_states.reload.update(this, context)
    if (main_track_states.reload.need_ammo == 2 or not context:hasAmmoToConsume()) then
        context:trigger(main_track_states.reload.retreat)
    else
        local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
        -- 等待 intro 结束，然后循环播放 loop 动画
        if(context:isHolding(track) and main_track_states.reload.need_ammo < 2) then
            if (main_track_states.reload.need_ammo == 0 and getBulletCount(context) ~= 3) then
                if(main_track_states.reload.emptyload == 1 or getBulletCount(context) == 1) then
                    context:runAnimation("reload_load", track, false, PLAY_ONCE_HOLD, 0.2)
                else
                    context:runAnimation("reload_load_2round", track, false, PLAY_ONCE_HOLD, 0.2)
                end
            end
            main_track_states.reload.need_ammo = main_track_states.reload.need_ammo + 1
        end
    end
end

...
function idle_state.transition(this, context, input)
    if (input == INPUT_RELOAD) then
        if (context:getAmmoCount() <= 0) then
            context:runAnimation("reload_empty", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
        else
            idle_state.empty = context:getCurrentTimestamp()
            context:runAnimation("reload_tactical", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
...
function idle_state.update(this, context)
    if (context:getCurrentTimestamp() < idle_state.empty + 1500) then
        context:setAnimationProgress(context:getTrack(STATIC_TRACK_LINE, BOLT_CAUGHT_TRACK), 1.1 - context:getAmmoCount()/context:getMaxAmmoCount(), false)
    else
        idle_state.empty = -1
    end
end

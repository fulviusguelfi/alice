...
local function getReloadTimingFromParam(param)
    ...
    local pipe_load = param.pipe_load * 1000
    local pipe_load_feed = param.pipe_load_feed * 1000
    -- Check if any timing is nil
    if (intro == nil or intro_empty == nil or loop == nil or ending == nil or ending_empty == nil or ending_empty_feed == nil or loop_feed == nil or pipe_load == nil or pipe_load_feed == nil) then
        return nil
    end
    return intro, intro_empty, loop, ending, ending_empty, ending_empty_feed, loop_feed, pipe_load, pipe_load_feed
end

function M.tick_reload(api)
    -- Get all timings from script parameter in gun data
    local param = api:getScriptParams();
    local intro, intro_empty, loop, ending, ending_empty, ending_empty_feed, loop_feed, pipe_load, pipe_load_feed = getReloadTimingFromParam(param)
    ...
    if (reloaded_count >= 0) then
        ...
        if(reloaded_count >= 3 and api:getMagExtentLevel() == 1) then
            base_time = (math.floor(reloaded_count / 3) * pipe_load) + (reloaded_count % 3) * loop
        end
        if (cache.needed_count - reloaded_count >= 3 and api:getMagExtentLevel() == 1) then
            base_time = base_time + pipe_load_feed
        else
        ...
        while (base_time < reload_time) do
            ...
            if (cache.needed_count - reloaded_count >= 3 and api:getMagExtentLevel() == 1) then
                ...
                base_time = base_time + pipe_load
                api:putAmmoInMagazine(api:isReloadingNeedConsumeAmmo() and api:consumeAmmoFromPlayer(3) or 3)
            else

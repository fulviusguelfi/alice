...
local function getReloadTimingFromParam(param)
    ...
    local intro_fanning = param.intro * 1000
    ...
    if (intro == nil or intro_fanning == nil or intro_empty == nil or loop == nil or ending == nil or loop_feed == nil) then
        return nil
    end
    return intro, intro_fanning, intro_empty, loop, ending, loop_feed
end

function M.tick_reload(api)
    -- Get all timings from script parameter in gun data
    local param = api:getScriptParams();
    local intro, intro_fanning, intro_empty, loop, ending, loop_feed = getReloadTimingFromParam(param)
